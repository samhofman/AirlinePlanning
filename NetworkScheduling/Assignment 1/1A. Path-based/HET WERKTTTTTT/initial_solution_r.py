...
obj = grb.quicksum(grb.quicksum(d(k)*cp(k,p)*fraction[k,p] for p in range(len(SP[k]))) for k in range(len(commodities))) + 1000000 * grb.quicksum( sl(a) * slack[a] for a in range(len(arcs)))
