...
### NEW GRAPH WITH UPDATED WEIGHTS ###    
    ...
    for a in range(len(arcs)):
        m.addConstr(grb.quicksum(grb.quicksum(d(k)*fraction[k,p]*delta(k,p,a) for p in range(len(delta_sp[k][p]))) for k in range(len(commodities))) - sl(a) * slack[a]
        ,
                                GRB.LESS_EQUAL,
                                u(a))
