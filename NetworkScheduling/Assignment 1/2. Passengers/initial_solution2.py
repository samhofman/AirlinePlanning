...
obj = grb.quicksum(grb.quicksum(d(k)*cp(k,p)*fraction[k,p] for p in range(len(SP[k][a])) for k in range(len(commodities))) + 1000 * grb.quicksum(slack[a] for a in range(len(arcs)))
