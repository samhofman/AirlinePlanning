...
for i in range(24):
    for j in range(24):
        if direct[i][j] > 0. or transfer[i][j] > 0.:
            seats_available = grb.LinExpr.getValue(grb.quicksum(flights[i][j][k] * seats[0][k] for k in range(5)))
            
            
            revenue = y(i,j)*distance(i,j)*(transfer[i][j]+direct[i][j])
            RPK = distance(i,j) * (transfer[i][j]+direct[i][j])
            ...
            KPI_values = np.array([i,j,distance(i,j),seats_available, transfer[i][j]+direct[i][j], '', ASK, RPK, '', cost, revenue, RASK, CASK, Yield])
            KPI = np.vstack( (KPI, KPI_values))
            

            tot_dist = tot_dist + distance(i,j)
            tot_seats = tot_seats + seats_available
            tot_pax = tot_pax + transfer[i][j]+direct[i][j]
...
with open('KPI_P3_results_OF.csv', 'wb') as myfile:
